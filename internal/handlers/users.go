...
func (cfg *ApiConfig) HandleUpdateUsers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		cfg.Logger.Warn("missing or invalid Authorization header",
			"path", r.URL.Path,
			"method", r.Method,
			"error", err,
		)
	...
	var req UserReq
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		cfg.Logger.Warn("invalid update user request body",
			"path", r.URL.Path,
			"method", r.Method,
			"error", err,
		)
	...
	updatedUser, err := cfg.Service.UpdateUsers(r.Context(), jwtToken, req.Email, req.Password)
	if err != nil {
		if errors.Is(err, services.ErrUnauthorized) {
			cfg.Logger.Warn("unauthorized update user request",
				"path", r.URL.Path,
				"method", r.Method,
				"error", err,
			)
		...
		cfg.Logger.Error("failed to update user",
			"path", r.URL.Path,
			"method", r.Method,
			"email", req.Email,
			"error", err,
